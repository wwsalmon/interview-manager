...
export default function Interview({dir, selected}: {dir: string, selected: string}) {
    ...
    return (
        <div className="h-screen flex flex-col border-l border-t">
            ...
            <div className="flex px-4 flex-grow-1" style={{height: "calc(100vh - 96px)"}}>
                <div className="w-1/2 flex-shrink-0 flex-grow-0 pr-4 border-r h-full overflow-auto">
                    <p className="text-xs uppercase font-bold my-4">Body</p>
                    <Textarea value={body} setValue={setBody} placeholder="Transcript, main content, etc."/>
                </div>
                <div className="w-1/2 flex-shrink-0 flex-grow-0 pl-4 h-full overflow-auto">
                    <p className="text-xs uppercase font-bold my-4">Notes</p>
